...
DROP SEQUENCE IF EXISTS tao.user_status_id_seq CASCADE;
-- create sequence
CREATE SEQUENCE tao.user_status_id_seq
	INCREMENT BY 1 MINVALUE 1 NO MAXVALUE START WITH 1 NO CYCLE;

-- assign sequence to column
ALTER TABLE tao.user_status ALTER COLUMN id SET DEFAULT nextval('tao.user_status_id_seq');

-- assign column to sequence
ALTER SEQUENCE tao.user_status_id_seq OWNED BY tao.user_status.id;
