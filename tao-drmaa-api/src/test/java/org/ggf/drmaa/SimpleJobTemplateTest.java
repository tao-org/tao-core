...
public class SimpleJobTemplateTest {
    ...
    public void testToString() throws DrmaaException {
        ...
        assertEquals("arg1", jt.getArgs().get(0));
        assertEquals("arg2", jt.getArgs().get(1));
        assertFalse(jt.getBlockEmail());
        assertEquals("myCategory", jt.getJobCategory());
        assertEquals("/usr/bin/csh", jt.getJobEnvironment().get("SHELL"));
        assertEquals("/tmp:/usr/bin", jt.getJobEnvironment().get("PATH"));
        assertEquals(jt.HOLD_STATE, jt.getJobSubmissionState());
        assertTrue(jt.getJoinFiles());
        assertEquals("-l arch=sol-sparc64", jt.getNativeSpecification());
        ...
        assertTrue(jt.getJoinFiles());
        assertEquals("-l arch=sol-sparc64", jt.getNativeSpecification());
        assertEquals(19, jt.getStartTime().get(Calendar.HOUR_OF_DAY));
        assertEquals(10, jt.getStartTime().get(Calendar.MINUTE));
        assertEquals(49, jt.getStartTime().get(Calendar.SECOND));
    }
}
